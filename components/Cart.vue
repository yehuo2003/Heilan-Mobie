<template>
  <div class="app_cart">
    <div class="mui-card">
      ...
      <div class="mui-card-footer" v-show="!isShow">
        <span>
          合计(不含运费)：
          <b>￥ {{(getSubtotal).toFixed(2)}}</b>
        </span>
        <mt-button type="danger" @click="faq">结算</mt-button>
      </div>
      <div class="card-null" v-show="isShow">
        <router-link to="/cate">
          <img src="http://127.0.0.1:3000/img/home/cart.png">
        </router-link>
      </div>
    </div>
  </div>
